...
void lcd_init (lcd_t *lcd)
{
	...
	lcd_cmd (lcd, (lcd_cmd_e)(LCD_CMD_FUNCTION_SET | DISPLAY_DL_8BIT));
	_platform_delay_ms(10);  	// wait for >4.1ms
	lcd_cmd (lcd, (lcd_cmd_e)(LCD_CMD_FUNCTION_SET | DISPLAY_DL_8BIT));
	...
	if (lcd->interface == LCD_INTERFACE_4BIT){
		...
		lcd_cmd (lcd, (lcd_cmd_e)(LCD_CMD_FUNCTION_SET | DISPLAY_DL_4BIT));  // 4bit mode
  ...
  // dislay initialisation
	...
	lcd_cmd (lcd, (lcd_cmd_e)(LCD_CMD_FUNCTION_SET | dispDl | DISPLAY_N_2LINE | dispFont)); // Function set --> DL=0 or 1, N = 1 (2 line display) F = 0 (5x8 characters)
	...
	_platform_delay_ms(5);
	lcd_cmd (lcd, (lcd_cmd_e)(LCD_CMD_DISP_CONTROL | lcd->_cursor)); //Display on/off control --> D = 1, C and B = 0. (Cursor and blink, last two bits)
